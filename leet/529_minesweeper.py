        ...
        for mi in (-1, 0, 1):
            for mj in (-1, 0, 1):
                ...
                if 0 <= ni < w and 0 <= nj < h:
                    neighbors_indices.append((ni, nj))
                    if board[ni][nj] in ('M', 'X'):
                        mines += 1
        if not mines:
            board[i][j] = 'B'
            for ni, nj in neighbors_indices:
                if board[ni][nj] in ('M', 'E'):
                    board = self.updateBoard(board, (ni, nj))
        else:
            board[i][j] = str(mines)
        return board
...
@pytest.mark.parametrize('board,click,expected', TESTCASES)
